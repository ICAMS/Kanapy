...
def particle_grow(sim_box, Ellipsoids, periodicity, nsteps):
    ...
    .. note:: :meth:`input_output.write_dump` function is called at each time step of the simulation to
    ...
    printProgressBar(0, nsteps, prefix='    Progress:', suffix='', length=50)
    ...
    while True:
        ...
        # Update Progress Bar
        printProgressBar(sim_box.sim_ts, nsteps,
                         prefix='    Progress:', suffix='', length=50)

        # IF simulation reaches the defined number of time steps - Exit packing, ELSE continue
        if sim_box.sim_ts == nsteps:
            break
        else:
            sim_box.sim_ts += 1
...
def packingRoutine():
    ...
    .. note:: Particle, RVE and simulation data are read from the JSON files generated by :meth:`input_output.particleStatGenerator`.
