...
nvox_var1 = tk.IntVar(value=40)
size_var1 = tk.IntVar(value=30)
periodic_var1 = tk.BooleanVar(value=True)

eq_diameter_sig = tk.DoubleVar(value=1.0)
eq_diameter_scale = tk.DoubleVar(value=7.0)
eq_diameter_loc = tk.DoubleVar(value=2.0)
eq_diameter_min = tk.DoubleVar(value=4.0)
eq_diameter_max = tk.DoubleVar(value=11.0)

aspect_ratio_sig = tk.DoubleVar(value=1.0)
aspect_ratio_scale = tk.DoubleVar(value=1.5)
aspect_ratio_loc = tk.DoubleVar(value=0.5)
aspect_ratio_min = tk.DoubleVar(value=0.9)
...
def add_label_and_entry(row, label_text, entry_var, entry_type="entry", bold=True, options=None):
    ...
    if entry_type == "entry":
        ttk.Entry(main_frame1, textvariable=entry_var, width=20, font=("Helvetica", 12)).grid(row=row, column=1, sticky='e')
...
ttk.Label(main_frame1, text="Equivalent Diameter Parameters", font=("Helvetica", 12, "bold")).grid(row=5, column=0, columnspan=2, pady=(10, 0), sticky='w')
...
ttk.Label(main_frame1, text="Aspect Ratio Parameters", font=("Helvetica", 12, "bold")).grid(row=11, column=0, columnspan=2, pady=(10, 0), sticky='w')
...
ttk.Label(main_frame1, text="Tilt Angle Parameters", font=("Helvetica", 12, "bold")).grid(row=17, column=0, columnspan=2, pady=(10, 0), sticky='w')
...
def create_and_plot_microstructure():
    ...
    ms = knpy.Microstructure(descriptor=ms_elong, name=f"{matname}_{texture}_texture")
    ms.init_RVE()
    fig = ms.plot_stats_init()
...
def create_rve_and_plot():
    ...
    ms.pack()
    ms.voxelize()
    fig = ms.plot_voxels(show = False, sliced=True)
...
def compare_statistics():
    ...
    ms.pack()
    ms.voxelize()
    fig = ms.plot_stats(show_plot=False, enhanced_plot=True)
    display_plot(fig, plot_type="stats")
...
button_statistics = ttk.Button(button_frame1, text="Statistics", style='TButton', command=create_and_plot_microstructure)
button_statistics.grid(row=0, column=0, padx=10)

button_compare_statistics = ttk.Button(button_frame1, text="Compare Statistics", style='TButton', command=compare_statistics)
...
entries2 = [ttk.Combobox(main_frame2, textvariable=texture_var2, values=['random', 'unimodal'], font=("Helvetica", 12), width=20),
...
create_orientation_button = ttk.Button(button_frame2, text="Create Orientation", style='TButton', command=create_orientation_from_gui)
create_orientation_button.grid(row=0, column=1, padx=10)
write_files_button = ttk.Button(button_frame2, text="Write Abaqus Input Files", style='TButton', command=write_abaqus_input_file_from_gui)
