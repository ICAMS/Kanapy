...
from kanapy.core.packing import *
...
def test_packingRoutine():
    ...
    with mock.patch('kanapy.core.packing.particle_generator') as mocked_method:
        packingRoutine([pd], True, 2, sb, save_files=False)
        assert mocked_method.call_count == 1

    # Test if the 'particle_grow' function is called once
    with mock.patch('kanapy.core.packing.particle_grow', return_value=(None, None)) as mocked_method:
        packingRoutine([pd], True, 2, sb, save_files=False)
        assert mocked_method.call_count == 1
