# -*- coding: utf-8 -*-
"""Historical growth/polygon voxelization, retained for legacy workflows."""
import numpy as np
import itertools
import logging
...
def points_in_convexHull_legacy(Points: np.ndarray, hull: ConvexHull) -> np.ndarray:
    """
...
def assign_voxels_to_ellipsoid_legacy(
    ...
    while vf_cur < vf_target:
        ...
        for ellipsoid in Ellipsoids:
            ...
            results = points_in_convexHull_legacy(bbox_testPts, hull)
        ...
        reassign_shared_voxels_legacy(cooDict, Ellipsoids, voxel_dict)
...
def reassign_shared_voxels_legacy(
    cooDict: Mapping[Any, Any],
    Ellipsoids: list[Any],
...
def voxelizationRoutine_legacy(
    Ellipsoids: list[Any],
    mesh: Any,
    nphases: int,
    ...
    - :meth:`create_voxels`
    - :meth:`assign_voxels_to_ellipsoid_legacy`
    - :meth:`reassign_shared_voxels_legacy`
    ...
    Notes
    -----
    - Voxels shared by multiple ellipsoids are resolved using `reassign_shared_voxels_legacy`.
    ...
    def ell2vox():
        ...
        1. Calls `assign_voxels_to_ellipsoid_legacy` to assign voxels based on ellipsoid positions and target volume fraction
        ...
        """
        assign_voxels_to_ellipsoid_legacy(mesh.vox_center_dict, Ellipsoids, mesh.voxel_dict, vf_target=prec_vf)
        # Create element sets
        for ellipsoid in Ellipsoids:
