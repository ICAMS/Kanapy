...
Since both these algorithms are imlemented as MATLAB functions that calls other MTEX functions, the MATLAB and MTEX installation paths are required for Kanapy's texture module. Make sure to set up the texture module within kanapy by running the command: 
